...
window.addEventListener('message', function(event) {
    ...
    if (item.response == 'openTarget') {
        ...
        $.each(item.data, function (index, item) {
            $(".target-label").append("<div class='target-item' id='target-"+index+"'<li><span class='target-icon'><i class='"+item.icon+"'></i></span>&nbsp&nbsp"+item.label+"</li></div>");
            // $("#target-"+index).hover((e)=> {
            //     $("#target-"+index).css("color",e.type === "mouseenter"?"rgb(205,205,255)":"whitesmoke")
            // })
        ...
        // $(".target-eye").css("color", "rgb(205,205,255)");
